...
class Permissions extends CI_Controller {
	...
	public function index()
	{
		...
		if(!$this->permission->has_permission("viewall_permission")){
			echo "No permissions";
			return;
		}
	...
	function create()
	{
		...
		$has_permision = $this->permission->has_permission("create_permission");
 ...
 */
 			// Set validaiton rules
				...
				$this->form_validation->set_rules("permissionDesc", "Permission Description", "trim|required|max_length[250]");
				$this->form_validation->set_rules("permissionCode", "Permission Code", "trim|required|max_length[25]");
	...
	function update()
	{
		if(!$this->permission->has_permission("edit_permission")){
			echo "No permissions";
			return;
		}
	...
	function delete()
	{
		$has_permision = $this->permission->has_permission("delete_permission");
		if(!$has_permision){
			echo "No permissions";
			return;
		}
		...
		if($permission_id){
			...
			if ($has_error && $has_error['permissionCode'] == 1451) {
